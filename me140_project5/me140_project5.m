...
for s = 2:3 % two stages: hot shift reactor, cold shift reactor
    ...
    h_out = hEng(T_guess(s), 'co',comps_out_adi(1,s)) + hEng(T_guess(s), 'h2ovap',comps_out_adi(2,s)) + hEng(T_guess(s), 'co2',comps_out_adi(3,s)) + hEng(T_guess(s), 'h2',comps_out_adi(4,s));
    dh = h_out-h_in;   
    
    while abs(dh) > error
        T_guess(s) = T_guess(s) + dh/speedFactor;  % temp increases through reactors
        comps_out_adi(:,s) = compositionsFun(f_kp_WGS(T_guess(s)));
        h_out = hEng(T_guess(s), 'co',comps_out_adi(1,s)) + hEng(T_guess(s), 'h2ovap',comps_out_adi(2,s)) + hEng(T_guess(s), 'co2',comps_out_adi(3,s)) + hEng(T_guess(s), 'h2',comps_out_adi(4,s));
        dh = h_out-h_in
    end
